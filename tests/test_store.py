...
class TestStore:
    ...
    def test_get_order(self, create_store_approved):
        with allure.step("Создание и получение питомца с id=1"):
            store_id = create_store_approved['id']
        ...
        with allure.step("Проверка статус кода"):
            ...
            jsonschema.validate(response_json, STORE_SCHEMA)
        with allure.step("Проверка содержания данных заказа"):
            assert response_json['id'] == create_store_approved['id'], "id store не совпадает с ожидаемым"
            assert response_json['petId'] == create_store_approved['petId'], "petId store не совпадает с ожидаемым"
            assert response_json['quantity'] == create_store_approved[
                'quantity'], "quantity store не совпадает с ожидаемым"
            assert response_json['status'] == create_store_approved['status'], "status store не совпадает с ожидаемым"
            assert response_json['complete'] == create_store_approved[
                'complete'], "complete store не совпадает с ожидаемым"

    @allure.title("Удаление заказа по ID")
    def test_delete_store_for_id(self, create_store_approved):
        with allure.step("Создание и получение питомца с id=1"):
            store_id = create_store_approved['id']
